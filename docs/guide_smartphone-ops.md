...
claude.ai/code で以下をコピペするだけで、今日のデータのサマリーが生成・反映される。

### サマリー生成の定型文

```
https://github.com/daichi223-web/future-insight をクローンして。

docs/data/latest.json と docs/data/sns-blogs.json を読んで、
docs/data/daily-summary.json にデイリーサマリーを書き出して。

フォーマット:
- date: 今日の日付(YYYY-MM-DD)
- overview: 全体の1〜2文のサマリー
...
1. Claude が latest.json と sns-blogs.json を分析
2. カテゴリ別にサマリーを生成
3. daily-summary.json に書き出し
4. コミット＆プッシュ → GitHub Pages に自動反映
5. サイトの概要タブにサマリーが表示される
...
| デイリーサマリー生成 | claude.ai/code + 定型文コピペ | 1〜2分 |
